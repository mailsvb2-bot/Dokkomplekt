# Production audit addendum — v1.4.37

Ключевой релизный закон: программа поставляется как конструктор медицинских документов. Встроенного медицинского комплекта шаблонов нет; каждый врач/клиника/страна загружает свои Word-шаблоны. UI должен вести пользователя через простой путь: добавить шаблоны → распознать названия → создать кнопки → настроить имя папки → создать документы.

# Current universal-profile foundation audit — v1.4.1-universal-template-pack

Добавлена первая безопасная итерация универсализации: полевая модель, профиль/пак документов, DOCX-разметчик и UI-вход `Профили`. Текущий рабочий сценарий генерации документов не заменён и не сломан; новый слой пока отвечает за настройку и обучение профиля.

Статус: архитектурный фундамент готов к следующему шагу — динамическим кнопкам блока 03 и пользовательским DOCX-шаблонам с `{field.id}`.

Остаточный риск: это ещё не полноценный универсальный продукт для всех специальностей; требуется доработка Template Mapper, динамического renderer и Windows QA разметчика на реальных обезличенных документах.

Оценка после итерации: **9.2/10 как foundation для универсальной версии**, текущий production-сценарий остаётся release-candidate.
...
- Генерация дневников.
- Регрессии парсера: `Не работает`, `врач`, `врач`, дата `12.01.26`, районы через `\`.
...
Итерация закрыла не UI-косметику, а эксплуатационные риски: line-ending churn на Windows, нечёткие output_dir ошибки, повторные input DOCX, stale embedded-template cache, неточные логи открытия папки, самопроверку release ZIP и hardening GitHub Actions.

Оценка после итерации: **9.55/10**. До формального 10/10 остаётся только проверка реальным Windows EXE на живом рабочем компьютере с настоящим принтером и реальными больничными шаблонами.
