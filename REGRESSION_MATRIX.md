...
| --- | --- | --- |
| Product model | Doctor-owned constructor, no bundled medical DOCX/DOCM templates, no narrow-profile defaults; legacy fixed-template backend is opt-in compatibility only | `prod_audit.py`, `release_check.py`, `tests/test_regression_contour_baseline_v1486.py::test_no_bundled_templates_or_builtin_documents_returned` |
...
| Popup values | Numeric/requisite popup values reach generated DOCX documents and outrank scanner data; doctor-confirmed admission date and VK/MSE combined work-position are final overlays | `tests/test_button_management_popup_values_folder_v1484.py`, `tests/test_regression_contour_baseline_v1486.py::test_full_doctor_regression_replay_from_template_to_output` |
| UI/data priority | Doctor-confirmed UI/popup state is the final overlay over parser/scanner/profile values, including repeated primary-document reparsing before render | `tests/test_contextual_role_disambiguation_v1482.py::test_doctor_confirmed_ui_values_are_final_overlay_for_custom_case`, `tests/test_regression_contour_baseline_v1486.py` |
...
| Diary behavior | Diary date templates and diary texts remain separate choices; useful `diary-filler` donor mechanics are preserved without restoring old bundled templates | `tests/test_diary_filler_donor_parity_v1490.py`, `smoke_full_patient_replay.py`, `smoke_desktop_diary_workflow.py`, `release_check.py` |
