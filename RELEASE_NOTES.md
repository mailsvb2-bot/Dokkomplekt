# Release notes — v1.4.100_template_residue_guard_hotfix

Internal release contract: template residue guard.

## Fixed

- Введён общий post-render template residue guard: неизменившийся динамический медицинский текст из donor DOCX не может перейти в документ нового пациента без поддержки каноническими данными текущего случая.
- Проверка привязана к семантическим разделам: чужую жалобу нельзя случайно сохранить только потому, что в ней совпал МКБ, дата или значение из другого поля.
- Структурный и служебный boilerplate сохраняется; слово `Пациент` само по себе больше не считается пациентским payload.
- Убран глобальный gender-pass по всему DOCX. Род адаптируется в канонических пациентских полях и только в явно принадлежащих renderer фразах, поэтому статический Word-текст не превращается в грамматически сломанные конструкции.
- Семантическое block-fill разворачивание ограничено main body. Header/footer остаются отдельными Word stories и не могут быть поглощены клиническим разделом.
- В выписном эпикризе устаревшая рекомендация про академический отпуск заменяется на точную строку `Рекомендовано: наблюдение у районного психиатра, приём препаратов`.
- Из «Совместного осмотра» удаляется старый пациентский текст жалоб, включая пользовательский пример `Пациентка предъявляет жалобы на апатию, отсутствие сил, нарушенный сон.`.
- Добавлены built-in, doctor-owned, adversarial и footer DOCX-регрессии этого класса ошибок.

# Release notes — v1.4.99_embedded_diary_text_source_hotfix

## Fixed

- Исправлен embedded diary text source propagation: doctor-owned кнопка дневников больше не требует повторно внешний файл «Тексты», если клинические тексты уже находятся внутри самого DOCX-шаблона врача.
- Подготовка дневников и «Мастер дневников» теперь используют один и тот же подтверждённый источник текстов, поэтому две части программы больше не противоречат друг другу.
- Ошибка мастера больше не маскируется общей фразой «проверьте дату, тексты и принцип»: при реальном недостающем поле показывается конкретная причина.
- Добавлена регрессия сценария со скриншота: 30.09.2025 → 05.06.2026, doctor-owned «Дневн», пустой внешний `status_files`, тексты внутри шаблона; дополнительно проверяется реальный DOCX renderer.
