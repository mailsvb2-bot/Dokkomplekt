...
def _fill_text_diary_batch(
    *,
    ...
    if sick_leave_dynamic_epicrisis:
        epi_input = DynamicEpicrisisInput(patient_name, birth_date, sick_leave_from or admission_value, complaints, treatment, profile_status, treatment_correction)
    ...
    *,
    ...
    for n, src_path in enumerate(diary_file_paths, start=1):
        ...
        result = fill_diary_file(
            dst,
            statuses,
            start_idx=0 if reset_each_file else idx,
            repeat_statuses=repeat_statuses,
