...
def parse_full_datetime(text: str) -> datetime:
    """Parse admission date with optional time.

    Date-only input remains valid.  If a time is present (for hourly diary
    profiles), it becomes the base moment for +N hour diary rows.
    """
    value = normalize_text(text)
    match = re.match(r"^(.+?)(?:\s+(\d{1,2})[:.](\d{2}))?$", value)
    if not match:
        return datetime.combine(parse_full_date(value), time(hour=0, minute=0))
    day_part = match.group(1).strip()
    parsed_date = parse_full_date(day_part)
    if match.group(2) is None:
        return datetime.combine(parsed_date, time(hour=0, minute=0))
    hour = int(match.group(2))
    minute = int(match.group(3))
    if hour > 23 or minute > 59:
        raise ValueError("Время поступления должно быть в формате ЧЧ:ММ")
    return datetime.combine(parsed_date, time(hour=hour, minute=minute))

def parse_admission_month_year(text: str) -> tuple[int, int]:
    value = normalize_text(text)
    try:
        admission_date = parse_full_datetime(value).date()
