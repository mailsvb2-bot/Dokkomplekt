...
# dynamic UI/settings attributes are provided by CombinedMedicalDiaryApp composition
...
class DiaryTemplateDiscoveryMixin:
    ...
    def _is_docx_like_template_file(path: str | Path) -> bool:
        ...
        try:
            ...
            try:
                ...
                record_soft_exception("diary_template_discovery.docx_like_zip", exc, detail=str(p))
                return False
        except Exception as exc:
            record_soft_exception("diary_template_discovery.docx_like_file", exc, detail=str(path))
            return False
    ...
    def _template_filename_day(path: str | Path) -> int | None:
        ...
        try:
            ...
            record_soft_exception("diary_template_discovery.template_filename_day", exc, detail=str(path))
            return None
    ...
    def _iter_diary_template_docx_files(self, folder: str | Path) -> list[Path]:
        ...
        try:
            ...
            try:
                ...
                record_soft_exception("diary_template_discovery.iter_cache_key", exc, detail=str(root))
            ...
            def add_file(path: Path) -> None:
                try:
                    ...
                    record_soft_exception("diary_template_discovery.iter_file_key", exc, detail=str(path))
                    key = str(path)
                if key not in seen:
                    seen.add(key)
                    files.append(path)
            ...
            def walk(current: Path, depth: int) -> None:
                ...
                try:
                    ...
                    record_soft_exception("diary_template_discovery.walk_iterdir", exc, detail=str(current))
                    return
            ...
            record_soft_exception("diary_template_discovery.collect", exc, detail=str(folder))
            return []
    ...
    def _template_content_first_day(self, path: str | Path) -> int | None:
        ...
        try:
            ...
            record_soft_exception("diary_template_discovery.content_day_stat", exc, detail=str(path))
            p = Path(path)
            cache_key = (str(p), 0, -1)
        ...
        try:
            ...
            record_soft_exception("diary_template_discovery.content_day_read", exc, detail=str(path))
            if isinstance(cache, dict):
        ...
        for pattern in (
            ...
            if match:
                try:
                    ...
                    record_soft_exception("diary_template_discovery.content_day_parse", exc, detail=str(path))
                    if isinstance(cache, dict):
                        cache[cache_key] = None
                    return None
        if isinstance(cache, dict):
    ...
    def _folder_contains_numbered_diary_templates(self, folder: str | Path) -> bool:
        ...
        try:
            ...
            record_soft_exception("diary_template_discovery.folder_contains", exc, detail=str(folder))
            return False
    ...
    def _is_named_diary_templates_folder(folder: str | Path) -> bool:
        ...
        try:
            ...
            record_soft_exception("diary_template_discovery.folder_name", exc, detail=str(folder))
    ...
    def _named_diary_template_dirs_near(self, root: str | Path) -> list[Path]:
        ...
        try:
            ...
            record_soft_exception("diary_template_discovery.named_near_root", exc, detail=str(root))
            return []

        starts: list[Path] = []
        for candidate in (base, base.parent, base.parent.parent, Path(__file__).resolve().parent, Path.cwd()):
            try:
                if candidate.exists() and candidate.is_dir():
                    starts.append(candidate)
            except Exception as exc:
                record_soft_exception("diary_template_discovery:272", exc)
        ...
        for start in starts:
            ...
            try:
                ...
                record_soft_exception("diary_template_discovery:286", exc)
            for candidate in candidates:
                try:
                    key = str(candidate.resolve())
                except Exception as exc:
                    record_soft_exception("diary_template_discovery.candidate_key", exc, detail=str(candidate))
                    key = str(candidate)
                if key in seen:
                    continue
                seen.add(key)
    ...
    def _discover_numbered_diary_template_dirs_near(self, root: str | Path) -> list[Path]:
        ...
        try:
            ...
            record_soft_exception("diary_template_discovery.numbered_near_root", exc, detail=str(root))
            return []

        candidates: list[Path] = []
        for start in (base, base.parent, base.parent.parent):
            if not start.exists() or not start.is_dir():
                continue
            candidates.append(start)
            try:
                for child in list(start.iterdir())[:160]:
                    if child.is_dir():
                        candidates.append(child)
            except Exception as exc:
                record_soft_exception("diary_template_discovery:323", exc)
        result: list[Path] = []
        seen: set[str] = set()
        for candidate in candidates:
            try:
                ...
                record_soft_exception("diary_template_discovery.numbered_candidate_key", exc, detail=str(candidate))
                key = str(candidate)
            if key in seen:
                continue
            seen.add(key)
    ...
    def _candidate_numbered_diary_template_dirs(self) -> list[Path]:
        ...
        def add_folder(candidate: str | Path, *, require_name: bool = True) -> None:
            ...
            try:
                ...
                record_soft_exception("diary_template_discovery.add_folder", exc, detail=str(candidate))
                return
            if key not in seen:
                seen.add(key)
        ...
        roots: list[str] = []
        primary_path = selected_primary_document_path_text(self)
        if primary_path:
            roots.append(primary_path)
