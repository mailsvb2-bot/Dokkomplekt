...
def apply_diary_entries(
    ...
    *,
    ...
    """Implement the apply_diary_entries workflow with validation, UI state updates and diagnostics."""
    idx = start_idx
    stats = {
        "filled_rows": 0,
        "detected_rows": 0,
    ...
    for entry_index, ((row, diary_col, day_col, month_year_col, hospitalization_day_col), entry) in enumerate(zip(data_entries, dated_entries)):
        ...
        if hospitalization_day_col is not None and len(row.cells) > hospitalization_day_col:
            # Real diary templates often already contain the «День госпитализации»
            # sequence (2, 3, 4, 7, 11...).  Preserve doctor-owned values and
            # fill only blank form rows; otherwise we destroy the template's
            # intended schedule.
            hosp_cell = row.cells[hospitalization_day_col]
            if not str(hosp_cell.text or "").strip():
                fill_text_cell(hosp_cell, str(entry_index + 1), alignment=WD_ALIGN_PARAGRAPH.CENTER)
        ...
        if time_text and not adapted_status.startswith(time_text):
            adapted_status = f"{time_text}. {adapted_status}"
        stats["gender_replacements"] += changed
