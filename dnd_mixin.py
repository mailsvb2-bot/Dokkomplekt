...
from diary_constants import DIR_DIARY_TEMPLATES, DIR_DIARY_TEXTS
from medical_constants import DIR_EPI
...
class DragDropMixin:
    ...
    def _register_tkinterdnd_drop_targets(self) -> bool:
        try:
            ...
            record_soft_exception("dnd_mixin.import_tkinterdnd2", exc)
            return False
        ...
        for widget in widgets:
            ...
            try:
                ...
                record_soft_exception("dnd_mixin.register_widget", exc, detail=repr(widget)[:200])
                continue
        return registered
    ...
    def _parse_drop_event_data(self, data: str) -> list[str]:
        ...
        try:
            ...
            record_soft_exception("dnd_mixin.parse_drop_event_tcl", exc, detail=data[:300])
    ...
    def _handle_dropped_files(self, paths: list[str]) -> None:
        """Implement the _handle_dropped_files workflow with validation, UI state updates and diagnostics."""
        primary_path = ""
        epi_path = ""
        ...
        if labs_paths:
            try:
                ...
                block = extract_labs_from_file(labs_paths[0], date_policy=self.labs_date_policy_var.get(), default_date=current_semantic_date(self, "admission_date"))
                self.labs_text_var.set(block.text)
                self.labs_source_path_var.set(block.source)
                self.labs_without_var.set(False)
                self._log(f"\n✅ Загружены анализы: {Path(labs_paths[0]).name}.\n")
            except Exception as exc:
                record_soft_exception("dnd_mixin.apply_labs_file", exc)
                ignored.append(Path(labs_paths[0]).name)
        first_known = primary_path or epi_path or (status_paths[0] if status_paths else "") or (diary_paths[0] if diary_paths else "") or (numbered_template_dirs[0] if numbered_template_dirs else "") or (diary_text_dirs[0] if diary_text_dirs else "") or (labs_paths[0] if labs_paths else "")
        ...
        recognized = sum(bool(x) for x in [primary_path, epi_path]) + len(status_paths) + len(diary_paths) + len(numbered_template_dirs) + len(diary_text_dirs) + len(labs_paths)
        if recognized:
    ...
    def _read_text_snippet_for_classification(path: Path, *, limit: int = 65536) -> str:
        ...
        try:
            ...
            record_soft_exception("dnd_mixin.read_text_snippet", exc, detail=str(path))
            return ""
    ...
    def _classify_dropped_file(self, path: str) -> str:
        """Implement the _classify_dropped_file workflow with validation, UI state updates and diagnostics."""
        p = Path(path)
        if p.is_dir():
            ...
            try:
                ...
                record_soft_exception("dnd_mixin:174", exc)
            return "unknown"
        ...
        if suffix == ".txt":
            ...
            if "эпи" in stem_low or txt_low.strip().startswith("эпи") or "эпи:" in txt_low or "эпидемиологическ" in txt_low:
                return "epi"
            try:
                from medical_renderer_labs import looks_like_labs_source_text
                if "анализ" in stem_low or looks_like_labs_source_text(txt, strong_only=True):
                    return "labs"
            except Exception as exc:
                record_soft_exception("dnd_mixin.classify_txt_labs", exc)
            return "unknown"
        if suffix not in {".docx", ".docm"}:
            if suffix == ".csv":
                try:
                    ...
                    record_soft_exception("dnd_mixin.classify_csv_labs", exc)
            return "unknown"
        ...
        try:
            ...
            record_soft_exception("dnd_mixin.extract_text_for_classification", exc, detail=str(path))
            text = ""
        ...
        try:
            ...
            record_soft_exception("dnd_mixin:206", exc)
        ...
        try:
            ...
            record_soft_exception("dnd_mixin:213", exc)
        ...
        try:
            ...
            record_soft_exception("dnd_mixin:221", exc)

        if "эпи" in low or "эпидемиологическ" in low:
            return "epi"
        try:
            from medical_renderer_labs import looks_like_labs_source_text
            if "анализ" in stem_low or looks_like_labs_source_text(text, strong_only=True):
                return "labs"
        except Exception as exc:
            record_soft_exception("dnd_mixin.classify_labs", exc)
        return "unknown"
