...
def extract_current_admission_date_from_primary_docx(path: str | Path) -> str:
    ...
    try:
        ...
        record_soft_exception("current_episode_dates.admission_title", exc, detail=str(path))
    try:
        text = extract_docx_text(path)
    except Exception as exc:
        record_soft_exception("current_episode_dates.admission_text", exc, detail=str(path))
        return title_date
...
def extract_current_discharge_date_from_primary_docx(path: str | Path) -> str:
    ...
    try:
        ...
        record_soft_exception("current_episode_dates.discharge_text", exc, detail=str(path))
        return ""
    return _date_after_explicit_label(text, CURRENT_DISCHARGE_LABELS)
