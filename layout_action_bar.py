...
from app_config import PANEL, TEXT
from diary_constants import DIARY_KIND


class LayoutActionBarMixin:
    def _build_action_bar(self, parent: tk.Frame) -> None:
        """Implement the _build_action_bar workflow with validation, UI state updates and diagnostics."""
        ...
        utility = tk.Frame(left, bg=PANEL)
        utility.grid(row=2, column=0, sticky="ew", pady=(self._px(7, 3), 0))
        utility.grid_columnconfigure(0, weight=1)
        utility.grid_columnconfigure(1, weight=1)
        utility.grid_columnconfigure(2, weight=1)
        utility.grid_columnconfigure(3, weight=1)
        utility.grid_columnconfigure(4, weight=1)
        utility.grid_columnconfigure(5, weight=1)
        self._small_neon_button(utility, text="Проверка", command=self.show_found_patient_data).grid(row=0, column=0, sticky="ew", padx=(0, self._px(3, 2)))
        self._small_neon_button(utility, text="Диагн.", command=self.show_installation_diagnostics_dialog).grid(row=0, column=1, sticky="ew", padx=(self._px(3, 2), self._px(3, 2)))
        self._small_neon_button(utility, text="Папка", command=self.configure_patient_folder_naming_dialog).grid(row=0, column=2, sticky="ew", padx=(self._px(3, 2), self._px(3, 2)))
        self._small_neon_button(utility, text="Пакет", command=self.batch_generate_documents_dialog).grid(row=0, column=3, sticky="ew", padx=(self._px(3, 2), self._px(3, 2)))
        self._small_neon_button(utility, text="Версия", command=self.check_updates_dialog).grid(row=0, column=4, sticky="ew", padx=(self._px(3, 2), self._px(3, 2)))
        self._small_neon_button(utility, text="Сброс", command=self.reset_settings_dialog).grid(row=0, column=5, sticky="ew", padx=(self._px(3, 2), 0))
    ...
    def show_installation_diagnostics_dialog(self) -> None:
        """Open the doctor-facing installation/watchdog self-check."""
        try:
            from installation_diagnostics import show_installation_diagnostics
            show_installation_diagnostics(self)
        except Exception as exc:
            record_soft_exception("layout_action_bar.installation_diagnostics", exc)
            messagebox.showwarning("Проверить программу", "Не удалось открыть проверку установки. Подробности записаны в диагностику.")
    ...
    def _redraw_selection_controls(self) -> None:
        ...
        for redraw in list(getattr(self, "_check_tile_redrawers", {}).values()):
            try:
                ...
                record_soft_exception("layout_action_bar:212", exc)
        for redraw in list(getattr(self, "_state_button_redrawers", [])):
            try:
                redraw(False)
            except Exception as exc:
                record_soft_exception("layout_action_bar:217", exc)

    def _on_custom_output_toggle(self, kind: str) -> bool:
        """Open the same missing-data popups for doctor-created block-03 buttons.

        Custom buttons are first-class document buttons.  A click on a doctor
        generated «Акт РВК» or «Выписной эпикриз» must therefore ask the same
        follow-up questions as the old built-in buttons, instead of waiting for
        the final «Создать» action and looking like the popup chain is broken.
        """

        try:
            from universal_main_documents import custom_document_id_from_kind

            document_id = custom_document_id_from_kind(kind)
            flags = self._custom_requirement_flags([document_id])
            self._active_custom_requirement_flags = flags
        except Exception as exc:
            record_soft_exception("layout_action_bar.custom_toggle_flags", exc, detail=kind)
            flags = {
                "diary": False,
                "regular": True,
                "discharge": False,
        ...
        if flags.get("diary"):
            return self._ensure_discharge_date(prompt_if_needed=True, force_prompt=True)
        if flags.get("rvk"):
            return self._prompt_rvk_details()
        if flags.get("discharge"):
            return self._prompt_discharge_output_requirements()
        if flags.get("commission") and not self._prompt_commission_details():
            return False
        if flags.get("vk_mse") and not self._prompt_vk_mse_details():
            return False
        if flags.get("sick_leave_vk") and not self._prompt_sick_leave_vk_details():
            return False

        include_medical = bool(flags.get("requires_diagnosis") or flags.get("requires_treatment") or flags.get("requires_labs") or flags.get("regular"))
        include_case = bool(flags.get("requires_case_number") or flags.get("regular"))
        include_discharge = bool(flags.get("requires_discharge_date"))
        return self._prompt_common_output_requirements(
            include_discharge_date=include_discharge,
            include_case_number=include_case,
            include_medical_details=include_medical,
            include_labs_block=bool(flags.get("requires_labs")),
        )
    ...
    def _on_output_toggle(self, kind: str) -> None:
        ...
        try:
            from universal_main_documents import is_custom_kind
            if is_custom_kind(kind):
                if not self._on_custom_output_toggle(kind):
                    self.output_vars[kind].set(False)
                self._update_selected_outputs_status()
                self._redraw_selection_controls()
                return
        except Exception as exc:
            record_soft_exception("layout_action_bar.custom_toggle", exc, detail=kind)

        if kind == "discharge":
            if not self._prompt_discharge_output_requirements():
                self.output_vars[kind].set(False)
        elif kind == DIARY_KIND:
            # Дневники всегда показывают popup даты выписки при включении: эта дата
            # определяет последнюю строку таблицы, поэтому врач должен видеть и
            # при необходимости подтвердить/изменить её даже после выписного.
            if not self._ensure_discharge_date(prompt_if_needed=True, force_prompt=True):
                self.output_vars[kind].set(False)
        elif kind == "rvk":
