...
from medical_primary_document_state import clean_primary_document_path
...
class LayoutSourcesMixin:
    ...
    def _source_drop_row(self, parent: tk.Frame, row: int) -> None:
        ...
        """Implement the _source_drop_row workflow with validation, UI state updates and diagnostics."""
    ...
    def _primary_selected_status_text(self, path: str | None = None) -> str:
        """Green one-line status for the chosen source plus confirmed discharge date."""
        fallback_path = clean_primary_document_path(self.navigation_path_var.get().strip()) if hasattr(self, "navigation_path_var") else ""
        raw_path = clean_primary_document_path(path or fallback_path)
        filename = Path(raw_path).name if raw_path else ""
        discharge = current_semantic_date(self, "discharge_date")
        max_chars = 38 if (self._compact_ui and discharge) else (46 if discharge else (54 if self._compact_ui else 82))
        ...
        text = f"{kind_text}: {filename}" if filename else kind_text
        if discharge:
            text += f"  •  Дата выписки: {discharge}"
        return text

    def _refresh_primary_selected_status(self) -> None:
        if not hasattr(self, "primary_selected_status_var"):
            return
        path = clean_primary_document_path(self.navigation_path_var.get().strip()) if hasattr(self, "navigation_path_var") else ""
        if not path:
            return
