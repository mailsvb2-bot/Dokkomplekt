...
EXCLUDE_DIRS = {
    ...
    "build", "dist", ".pytest_cache", ".ruff_cache", ".mypy_cache", "release", ".vscode", ".idea",
}
EXCLUDE_SUFFIXES = {".pyc", ".pyo", ".spec", ".bak", ".tmp", ".log"}
EXCLUDE_NAMES = {"startup_error.log", ".DS_Store", "Thumbs.db", "coverage.xml", ".coverage"}
...
def _assert_clean_archive(zf: zipfile.ZipFile) -> None:
    ...
    for name in names:
        ...
        if Path(name).name in EXCLUDE_NAMES:
            bad.append(name)
        if Path(name).name.startswith("ОТЧЁТ_") and Path(name).suffix.lower() == ".txt":
            bad.append(name)
        if Path(name).name.startswith("FIX_REPORT_v") and Path(name).suffix.lower() == ".md":
            bad.append(name)
    if bad:
...
def should_include(path: Path) -> bool:
    ...
    if path.name in EXCLUDE_NAMES:
        return False
    if path.name.startswith("FIX_REPORT_v") and path.suffix.lower() == ".md":
        return False
