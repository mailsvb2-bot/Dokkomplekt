...
class MedicalParserInlineMixin:
    ...
    def _parsed_field_value_is_only_label(field_name: str, value: str) -> bool:
        ...
        values like a position ``врач`` / ``врач-хирург``.
        """
    ...
    def _extract_inline(self, text: str, aliases: Sequence[str], *, field_name: str = "") -> str:
        ...
        for alias in aliases:
            ...
            for index, line in enumerate(lines):
                ...
                if not clean_value(value):
                    value = value_after_label_line(
                        lines,
                        index,
                        all_aliases=all_aliases,
                        allow_label_values=field_name in {"position", "doctor", "head"},
                    )
    ...
    def _ignore_inline_alias_match(line: str, alias: str, start: int, end: int) -> bool:
        ...
        if not re.match(r"\s*(?:[:№N#.-]|в\s+организации|$)", after, flags=re.IGNORECASE):
            return False
        # A table cell may contain only the label «Работает», with the value in
        # the next cell/line.  Do not reject it here; `_extract_inline()` will
        # look ahead conservatively and stop at the next label.
        return False
    ...
    def _cut_at_next_inline_marker(self, value: str, current_aliases: Sequence[str]) -> str:
        ...
        for alias in self._all_inline_aliases():
            ...
            if not m:
                continue
            alias_norm = normalize_match(alias)
            # В значении поля «Должность» слова «врач» / «врач-хирург»
            # являются реальной должностью, а не началом следующего поля «Врач».
            if "должность" in current_norm and alias_norm in {"врач", "врач психиатр", "врач-психиатр", "хирург", "терапевт"}:
                continue
            if m.start() < best:
                best = m.start()
