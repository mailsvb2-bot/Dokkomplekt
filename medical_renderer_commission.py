...
class MedicalRendererCommissionMixin:
    def render_commission(self, template_path: str | Path, output_path: str | Path, data: PatientData) -> None:
        ...
        if data.commission_date or data.commission_number:
            ...
            header = (
                ...
                f"Совместный осмотр врачебной комиссией № {data.commission_number}"
            ).rstrip()
        ...
        editor.replace_block(["В 3 отделение КДП поступает", "Поступает"], "Поступает:", data.admission, COMMISSION_MARKERS)
        ...
        editor.replace_block(["Психический статус при поступлении", "Психический статус"], "Профильный статус при поступлении:", data.mental_status, COMMISSION_MARKERS)
        ...
        self._replace_lab_lines(editor, dates, data=data, all_markers=COMMISSION_MARKERS)
        if data.epi_text:
    ...
    def render_admission_doctor_referral(self, template_path: str | Path, output_path: str | Path, data: PatientData) -> None:
        ...
        editor.replace_block(["На учёте у психиатров", "На учете у психиатров", "Профильное наблюдение"], "Профильное наблюдение:", data.psych_account, PRIMARY_MARKERS, allow_empty=True)
        ...
        editor.replace_block(["В 3 отделение КДП поступает", "Поступает"], "Поступает:", data.admission, PRIMARY_MARKERS, allow_empty=True)
        ...
        target_referral_line = f"По медицинским показаниям направляется на лечение в {TARGET_MEDICAL_FACILITY}"
        referral_done = False
        ...
        editor.replace_block(["Врач", "Врач-психиатр"], "Врач", data.doctor, PRIMARY_MARKERS, allow_empty=True)
        finalize_medical_document(doc, data)
        doc.save(str(output_path))
