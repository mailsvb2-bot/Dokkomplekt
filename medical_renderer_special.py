...
class MedicalRendererSpecialMixin:
    def render_vk_mse(self, template_path: str | Path, output_path: str | Path, data: PatientData) -> None:
        ...
        editor.replace_all_matching_paragraphs(["Проживает"], f"Проживает: {data.registered}")
        vk_work_parts = [
        ...
        editor.replace_block(["Психический статус при поступлении", "Психический статус"], "Профильный статус при поступлении:", data.mental_status, VK_MSE_MARKERS)
        if data.epi_text:
    ...
    def render_sick_leave_vk(self, template_path: str | Path, output_path: str | Path, data: PatientData) -> None:
        ...
        editor.replace_block(["Психический статус при поступлении", "Психический статус"], "Профильный статус при поступлении:", data.mental_status, SICK_LEAVE_VK_MARKERS)
        if data.epi_text:
    ...
    def render_rvk(self, template_path: str | Path, output_path: str | Path, data: PatientData) -> None:
        ...
        editor.replace_block(["Психический статус"], "Профильный статус:", data.mental_status, RVK_MARKERS)
        ...
        self._replace_lab_lines(editor, dates, data=data, all_markers=RVK_MARKERS)
        if data.epi_text:
