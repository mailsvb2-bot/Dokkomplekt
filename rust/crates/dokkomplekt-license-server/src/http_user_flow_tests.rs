...
async fn emulate_user_flow(app: Router, storage_backend: &str, database_connected: bool) {
    ...
    let order_body = json!({ "plan": "doctor_pro", "amount_rub": 3900, "machine_hash": "machine-a" });
    let (status, order) = call(app.clone(), Method::POST, "/api/orders".to_string(), Some(order_body)).await;
    assert_eq!(status, StatusCode::OK);
    ...
    let activate_body = json!({ "machine_hash": "machine-a" });
    let (status, activation) = call(app.clone(), Method::POST, format!("/api/orders/{order_id}/activate-machine"), Some(activate_body)).await;
    assert_eq!(status, StatusCode::OK);
...
#[tokio::test]
async fn memory_user_payment_license_flow() {
    let app = build_app(AppState::try_new(config(None)).unwrap());
    emulate_user_flow(app, "memory", false).await;
}

#[tokio::test]
async fn postgres_user_payment_license_flow_when_database_url_is_present() {
    ...
    let app = tokio::task::spawn_blocking(move || build_app(AppState::try_new(config(Some(database_url))).unwrap()))
        .await
        .unwrap();
    emulate_user_flow(app.clone(), "postgres", true).await;
    std::mem::forget(app);
}
