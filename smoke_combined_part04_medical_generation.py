...
if not _legacy_fixed_templates_available():
    print("LEGACY FIXED MEDICAL GENERATION SMOKE SKIPPED: doctor-owned production build ships no fixed templates")
else:
    ...
    assert "Экспертный анамнез: Работает в ООО Завод, в должности инженер. Больничный лист № 123456789. Срок лечения с 10.06.2026 по 11.06.2026, 2 дня. К труду с 12.06.2026." in combined_text
    ...
    phrase_data.diagnosis = "K35.8 Тест"
    ...
    assert not any("Целесообразна госпитализация" in extract_docx_text(path) for path in phrase_created), phrase_created
    ...
    def _make_user_contract_primary(path: Path) -> None:
        ...
        doc.add_paragraph("Зарегистрирован: город N, ул. Проверочная, д. 1")
        ...
        doc.add_paragraph("Профильный статус: Контактен, ориентирован, эмоционально напряжён.")
        ...
        doc.add_paragraph("На основании данных анамнеза жизни и заболевания, профильного статуса, данных клинических исследований был выставлен диагноз: K35.8 Острый аппендицит")
        doc.save(path)
    ...
    try:
        ...
        assert "K35.8 Острый аппендицит" in contract_discharge_text, contract_discharge_text
