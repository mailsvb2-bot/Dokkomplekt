...
def main() -> None:
    ...
    with TemporaryDirectory() as tmp:
        ...
        _doc(primary, ["История болезни № 123", "Пациент: Иванов Иван Иванович", "Дата поступления: 10.02.2026", "Дата выписки: 12.02.2026", "Диагноз: K35.8 Острый аппендицит", "Лечение: режим, наблюдение, терапия по назначению врача"])
        ...
        result = fill_diary_batch(status_files=[texts], diary_files=[template], output_dir=patient_dir, patient_name="Иванов И.И.", admission_value="10.02.2026", discharge_value="12.02.2026", remove_holiday_rows=False, force_final_diary=False)
    ...
    if os.name == "nt":
        _run_application_user_journey()
    else:
        print("APPLICATION USER JOURNEY SMOKE SKIPPED: Windows/Tk runtime required")
...
def _journey_doc(path: Path, paragraphs: list[str]) -> Path:
    document = Document()
    for text in paragraphs:
        document.add_paragraph(text)
    document.save(path)
...
def _build_profile(root: Path):
    ...
    diary_spec = replace(diary_spec, category="diaries", role_id="daily_diary", diary_schedule=DiaryScheduleSpec("daily", (1, 2, 3, 4), (), 1.0, "full_user_journey").to_dict())
...
def _build_app(primary_path: Path, patient_dir: Path, profile_path: Path, pack):
    ...
    app._settings = {"active_universal_profile": str(profile_path), "folder_naming": {"parts": ["surname_initials", "admission_month"], "date_format": "short", "doctor_confirmed": True}}
    ...
    for name in ("expert_work_org_var", "expert_position_var", "expert_sick_leave_from_var", "expert_sick_leave_number_var", "commission_date_var", "commission_number_var", "rvk_act_number_var", "rvk_military_commissariat_var", "rvk_work_position_var", "vk_date_var", "vk_protocol_number_var", "vk_protocol_date_var", "vk_mse_work_org_var", "vk_mse_position_var", "vk_mse_work_position_var", "sick_leave_vk_date_var", "sick_leave_vk_protocol_number_var", "sick_leave_vk_protocol_date_var", "sick_leave_vk_commission_date_var", "sick_leave_vk_work_org_var", "sick_leave_vk_position_var", "sick_leave_vk_work_position_var", "diary_treatment_correction_var"):
...
def _run_application_user_journey() -> None:
    with TemporaryDirectory(prefix="dokkomplekt-full-user-journey-") as raw:
        root = Path(raw)
        intake = root / "Выписанные пациенты"
        intake.mkdir()
        ...
        with (patch("diary_creation_wizard.confirm_diary_creation", return_value=True), patch("diary_creation_wizard.current_diary_calendar_schedule", return_value=_Schedule("daily", (1, 2, 3, 4), (), 1.0, "full_user_journey"))):
