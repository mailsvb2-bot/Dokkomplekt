...
from desktop_intake import primary_document_score
...
# 5) Labs popup stays simple but the scanners are visible.
fields_core = Path("dialog_fields_core.py").read_text(encoding="utf-8")
visible_labs_block = fields_core.split("def open_labs_selection_scanner", 1)[0]
...
# 6) Windows minimize/restore stays native-frame based.
...
# 7) Desktop intake patient folder is already final: do not nest another patient folder.
from actions_creation_foldering import ActionsCreationFolderingMixin

folder_fake = SimpleNamespace(_output_dir_auto_locked_to_patient=True, discharge_date_var=_Var(""))
...
# 8) Blank diary tables are valid doctor templates: date and text are filled.
...
# neutral ICD/lexical matching of doctor-owned filename variants.
import tempfile
...
with tempfile.TemporaryDirectory(prefix="selection_principles_") as tmp_dir:
    ...
    coded = tmp / "кодированные тексты дневников"
    coded.mkdir()
    coded_expected = {
        "дневники ВЭ K35 острый аппендицит.docx": "K35 Острый аппендицит",
        "дневники ВЭ I10 артериальная гипертензия.docx": "I10 Артериальная гипертензия",
        "дневники ВЭ J20 острый бронхит.docx": "J20 Острый бронхит",
        "дневники ВЭ E11 сахарный диабет.docx": "E11 Сахарный диабет 2 типа",
    }
    for filename in coded_expected:
        _empty_docx(coded / filename)
    for filename, diagnosis in coded_expected.items():
        matched = find_diary_text_file_for_diagnosis(coded, diagnosis)
        assert matched is not None, diagnosis
        assert matched.name == filename, (diagnosis, matched.name)

    diary_source = Path("diary_text_selection.py").read_text(encoding="utf-8").casefold()
    for forbidden in (
        "legacy_cognitive",
        "legacy_asthenic",
        "legacy_affective",
        ...
        assert forbidden not in diary_source

# 10) Existing hospitalization-day sequences from doctor templates are preserved.
...
# 11) Desktop-intake popup must expose bulk controls, cancellation and diary tile support.
