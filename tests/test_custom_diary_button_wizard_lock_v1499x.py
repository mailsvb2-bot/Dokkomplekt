...
def test_custom_diary_button_uses_same_wizard_as_diary_checkbox():
    ...
    assert "confirm_diary_creation(self, text_source_labels=text_source_labels)" in section
    assert "text_source_labels = self._ensure_diary_text_files_for_creation" in section
