...
from diary_batch import DynamicEpicrisisInput, build_dynamic_epicrisis_text, default_observation_diary_dates, fill_diary_batch, is_non_working_day
...
def test_diary_filler_donor_parity_supports_merged_status_cells_once(tmp_path: Path) -> None:
    ...
    assert extract_statuses_from_docx(merged_status) == ["Пациент был спокоен, жалоб активно не предъявлял, в беседе доступен, инструкции выполнял."]
