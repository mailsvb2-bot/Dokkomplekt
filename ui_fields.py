...
class UiFieldsMixin:
    ...
    def _refresh_discharge_date_visual(self) -> None:
        """Show the global discharge date as an explicit green confirmed field.

        The date is edited through the patient card and several popup windows.
        Without a single visual refresh hook the popup value could be stored
        correctly but still look like an ordinary/unconfirmed UI value.
        """
        entry = getattr(self, "_discharge_date_entry", None)
        value = ""
        try:
            value = current_semantic_date(self, "discharge_date")
        except Exception as exc:
            record_soft_exception("ui_fields.discharge_value", exc)
            value = ""
            ...
            if value:
                ...
                if parsed is not None:
                    try:
                        ...
                        color = SUCCESS if self._date_is_not_before_admission(normalized) else WARN
                    except Exception as exc:
                        record_soft_exception("ui_fields.discharge_date_compare", exc)
                        color = SUCCESS
                else:
                    color = WARN
            try:
                entry.config(fg=color, insertbackground=color if value else TEXT)
            except Exception as exc:
                record_soft_exception("ui_fields.refresh_discharge_date_state", exc)

        refresh_status = getattr(self, "_refresh_primary_selected_status", None)
    ...
    def _field(self, parent, label: str, variable: tk.StringVar, *, row: int, col: int, colspan: int = 1, placeholder: str = "") -> None:
        ...
        if variable is getattr(self, "discharge_date_var", None):
            self._discharge_date_entry = entry_canvas.entry  # type: ignore[attr-defined]
