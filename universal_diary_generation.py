...
from diary_batch import fill_diary_batch, read_statuses_from_files
...
# diaries with a real diary table may use an isolated template-preserving renderer;
# ordinary Word/text diary templates remain on the canonical semantic text route.
...
def render_diary_documents_from_pack(
    *,
    ...
    """Render selected custom diary buttons without changing their diary model.

    A doctor-owned template that actually contains a diary table keeps that Word
    layout through the isolated renderer. A text/paragraph template remains a
    text diary source and is generated by the canonical semantic text-calendar
    route (``diary_files=[]``, ``text_output=True``). The selected set is atomic:
    one failed diary invalidates and removes the whole generated subset.
    """
    ...
    for document in pack.documents:
        ...
        if not effective_status_files:
            ...
            continue
        try:
            if _uses_template_table_renderer(template):
...
def _uses_template_table_renderer(template: Path) -> bool:
    """Detect only a real diary table; paragraph templates stay text diaries."""

    try:
        source = ensure_docx_compatible(template, label="шаблон дневников")
        document = Document(str(source))
        for table in document.tables:
            for row in table.rows[:6]:
                texts = [" ".join((cell.text or "").replace("ё", "е").casefold().split()) for cell in row.cells]
                if any("дневник" in text or "наблюден" in text for text in texts):
                    return True
    except Exception as exc:
        record_soft_exception("universal_diary_generation.table_layout_probe", exc, detail=str(template))
    return False
...
def assert_universal_diary_generation_lock() -> None:
    ...
    if not CUSTOM_DIARY_USES_DOCTOR_TEMPLATE_RENDERER:
        raise AssertionError("Doctor-owned table diaries must preserve their Word template")
